class WhisperNotification < AWS::Record::HashModel
  ...
  def self.send_message(target_id, current_user, venue_id, notification_type, intro, message, timestamp, image_thumb_url, image_url, audio_url, client_side_id)
    origin_id = current_user.id.to_s
      ...
      if conversation.nil?
        ...
        if whisper_just_sent.blank?
          ...
          if !timestamp.nil?
            ...
            chat_message = ChattingMessage.create!(:grouping_id => timestamp_with_ns.to_i, :speaker_id => current_user.id, :whisper_id => whisper.id, :message => intro, :image_thumb_url => image_thumb_url, :image_url => image_url, :audio_url => audio_url, :created_at => timestamp_with_ns, :updated_at => timestamp_with_ns, :client_side_id => client_side_id)
          else
            ...
            chat_message = ChattingMessage.create!(:grouping_id => Time.now.to_i, :speaker_id => current_user.id, :whisper_id => whisper.id, :message => intro, :image_thumb_url => image_thumb_url, :image_url => image_url, :audio_url => audio_url, :client_side_id => client_side_id)
          end
          if n and notification_type == "2"
            time = Time.now
        ...
        if conversation.chatting_messages.count < 2 and current_user.id == conversation.origin_user_id
          ...
          if !timestamp.nil?
            ...
            chat_message = ChattingMessage.create!(:image_thumb_url => image_thumb_url, :image_url => image_url, :audio_url => audio_url, :grouping_id => grouping_id, :speaker_id => current_user.id, :whisper_id => conversation.id, :message => intro, :created_at => timestamp_with_ns, :updated_at => timestamp_with_ns, :client_side_id => client_side_id)
          else
            ...
            chat_message = ChattingMessage.create!(:image_thumb_url => image_thumb_url, :image_url => image_url, :audio_url => audio_url, :grouping_id => grouping_id, :speaker_id => current_user.id, :whisper_id => conversation.id, :message => intro, :client_side_id => client_side_id)
          end
          if chat_message and Rails.env == 'production'
            # :nocov:
            target_user = User.find_user_by_unique(target_id)
  ...
  def self.send_notification_330_level(id, type, total_votes, shout_id)
    deep_link = "yero://shouts/" + shout_id.to_s
