class WhisperNotification < AWS::Record::HashModel
  ...
  def self.whisper_sent(origin_user_id, target_user_id)
    ...
    puts "whisper sent"
    puts items.attributs.to_h.inspect
    if items.present? and items.count > 0
      return true
    else
      return false
  ...
  def send_push_notification_to_target_user(message)
    ...
    table = dynamo_db.tables['WhisperNotification']
    table.load_schema
    chat_items = table.items.where(:target_id).equals(target_user.id.to_s).where(:notification_type).equals("2").where(:viewed).equals(1)
    greeting_items = table.items.where(:target_id).equals(target_user.id.to_s).where(:notification_type).equals("1").where(:viewed).equals(1)
  ...
  def self.send_nightopen_notification(id)
    ...
    # Create a notification that alerts a message to the user, plays a sound, and sets the badge on the app
    notification = Houston::Notification.new(device: token)
    notification.alert = "Yero network is now online"
