module V20150930
  class ConversationsController < ApplicationController
    prepend_before_filter :get_api_token, only: [:create, :index, :update, :destroy, :show, :show_messages, :show_single_message, :create_single_message]
    before_action :authenticate_api_v2, only: [:create, :index, :update, :destroy, :show, :show_messages, :show_single_message, :create_single_message]
    ...
    def create_single_message
      ...
      if target_user
        time_1 = Time.now
        result = WhisperNotification.send_message(target_user.id, current_user, venue_id, notification_type, text_message, message, timestamp, image_thumb_url, image_url, audio_url, client_side_id)
        time_2 = Time.now
